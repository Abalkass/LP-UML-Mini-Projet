...
//		cat.clear();
	...
	public void testGetNomProduits_plusieursProduitsInseresOrdreAlphabetique() {
		...
		cat.addProduit("Raider", 12, 2);
		cat.addProduit("Treets", 10, 1);
		assertArrayEquals("récupère nom de plusieurs produits ajout€s dans ordre alphabétique", tab, cat.getNomProduits());
	}	
	...
	public void testGetNomProduits_plusieursProduitsInseresOrdreAleatoire() {
		...
		assertArrayEquals("récupère nom de plusieurs produits ajout€s dans ordre al€atoire (doivent être retourn€s dans l'ordre alphabétique)", tab, cat.getNomProduits());
	}
	...
	public void testToString_CatalogueAvecDesProduits_TotalAvecTroisChiffresApresVirguleArrondiInferieur() {
		...
		assertEquals("on affiche que deux chiffres aprés la virgule dans le prix unitaires TTC, mais le montant total TTC du catalogue est calcul€ avec les prix unitaires TTC non arrondis",resultatAttendu, cat.toString());
	}
	...
	public void testClear() {
		...
		cat.addProduit("Raider", 1, 10);
		cat.clear();
		assertTrue("le catalague a est supprimer", cat.getNomProduits().length == 0);
	}
